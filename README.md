...
/plugin install icu-clinical-consult@neel-plugins
```
...
**The adversarial panel.** `icu-clinical-consult` solves a different problem: a single reasoner finds a frame early and defends it, so asking for alternatives returns variations rather than genuine alternatives. It dispatches four specialists who cannot see each other's findings, then runs a cross-check whose actual output is the *disagreements*. Where two reviewers contradict each other is where the real question is.
...
| | Needed for |
|---|---|
| **Claude Code** with the Agent tool | The multi-agent panel. Without it it degrades to a single pass and says so. |
| **Web access** | Every verification path, and guideline retrieval. |
| **PubMed MCP** *(optional)* | Improves retrieval in `citation-verification` and `icu-clinical-consult`. |

All four work on any current Claude model. They were developed and tuned on Opus 5; nothing pins a model ID.

---

## The bundled dependency

`icu-clinical-consult` and `citation-verification` each ship a copy of `fabrication-audit` so they work standalone on install.

If you install both, also install `fabrication-audit` itself — that repo is canonical, and if the copies drift, it wins.

For maintainers, `scripts/sync-fabrication-audit.sh` checks and repairs drift across the bundles. Clone the repos into one directory, then:
